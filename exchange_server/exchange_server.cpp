...
int main(int argc, char *argv[]) {
    ...
    vanilla::config::config<exchange_config_data> config([&](exchange_config_data &d, boost::program_options::options_description &desc){
        desc.add_options()
            ...
            ("multi-bidder.port", po::value<unsigned short>(&port)->default_value(5000), "udp port for broadcast")
            ("multi-bidder.num_of_bidders", po::value<int>(&n_bid)->default_value(1), "number of bidders to wait for")
        ;
    });
    try {
    ...
    dispatcher.crud_match(boost::regex("/vanilla/auction/(\\d+)"))
              .post([&](http::server::reply & r, const http::crud::crud_match<boost::cmatch> & match) {
                  openrtb_handler.handle_post(r,match);
              });
    dispatcher.crud_match(boost::regex("/vanilla/(v2[.][2-4])/auction/(\\d+)"))
              .post([&](http::server::reply & r, const http::crud::crud_match<boost::cmatch> & match) {
                  openrtb_handler_v2.handle_post(r,match);
              });
    dispatcher.crud_match(boost::regex("/vanilla/multi-bidders/auction/(\\d+)"))
              .post([&](http::server::reply & r, const http::crud::crud_match<boost::cmatch> & match) {
