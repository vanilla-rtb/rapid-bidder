...
int main(int argc, char* argv[]) {
    ...
    vanilla::config::config<multi_exchange_handler_config_data> config([&](multi_exchange_handler_config_data &d, po::options_description &desc){
        desc.add_options()
            ("multi_buidder_exchange.log", po::value<std::string>(&d.log_file_name), "exchange_handler_test log file name log")
            ("multi_bidder_exchange.host", "multi_bidder_exchange_server Host")
            ("multi_bidder_exchange.port", "multi_bidder_exchange_server Port")
            ("multi_bidder_exchange.root", "multi_bidder_exchange_server Root")
            ("multi_bidder_exchange.concurrency", po::value<int>(&d.concurrency)->default_value(0), "concurrency")
            ("multi_bidder_exchange.timeout", po::value<int>(&d.handler_timeout)->required(), "multi_exchange_handler_timeout")
